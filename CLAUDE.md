...
- Conteúdo vive nas collections `projetos`, `depoimentos`, `sobre` e `familiaridade` em `src/content/` (as duas últimas são um arquivo cada em `src/content/home/`), como arquivos de dados com campos em pt-BR sem acento (tabelas em `CONTEXT.md`); `descricao` é texto simples e nenhum campo aceita HTML. Imagens em `src/assets/` (WebP ≤ 2560 px). O schema Zod é a verdade: build quebra se faltar campo, imagem ou se `ordem` repetir.
