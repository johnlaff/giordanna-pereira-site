...
- `public/_headers` — HSTS, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` e `Permissions-Policy` de toda resposta estática, e `X-Robots-Tag: noindex` no workers.dev (o endereço indexado é o domínio, ADR 0007); o adapter soma a ele o cache imutável de `/_astro/*`. As respostas do Worker levam os mesmos, de `src/headers-de-seguranca.ts`
