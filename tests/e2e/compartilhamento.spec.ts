...
const PUBLICAS = ['/', '/projetos', ...projetos.map(({ rota }) => rota), '/contato'];
...
test('as doze JPGs do handoff saíram da pasta pública', async ({ request }) => {
  // Os Cartões do Preview eram fotografias fixas; agora quem publica em og/ é o build.
  for (const antiga of ['/og/edificio-vitalis.jpg', '/og/mirante-cestes.jpg'])
    expect((await request.get(antiga)).status()).toBe(404);
});
