...
- O tempo do build está nas imagens: o Astro deriva uma variante AVIF e uma WebP por largura de cada imagem dos Projetos — cerca de 1900 arquivos e 150 MB, minutos a frio no runner do GitHub contra segundos com as variantes prontas (cada Projeto novo no CMS aumenta os três números). O AVIF é 96,6% desse trabalho, e é o esforço de compressão dele que decide o custo: o ADR 0009 baixou o esforço de 4 para 3 e cortou o build a frio a menos de um terço — de doze a vinte e quatro minutos no runner antes dele, e de 17 min 04 s para 4 min 22 s na medição de acervo inteiro que sustenta o ADR (2026-09-20, quatro núcleos). Os três jobs que constroem o site restauram `node_modules/.astro/assets` pela action de setup. A chave é presa ao que decide as variantes e a nada mais, em duas partes: o prefixo é como elas são comprimidas (`src/imagens.ts`, `src/servico-de-imagem.ts`) e o sufixo é quais imagens existem (`src/assets`). A reserva cai no prefixo, então uma imagem nova aproveita o cache anterior e o build gera só o que faltar, enquanto uma mudança de compressão não encontra reserva alguma e regera o acervo. A ordem importa porque o nome do arquivo gerado não registra com que compressão ele foi feito: com a compressão no sufixo, a reserva devolveria as variantes antigas com os nomes certos e o build as daria por prontas. O resto do conteúdo fica de fora para que uma edição de texto no CMS não invalide 150 MB de imagem. Uma imagem nova custa o build cheio uma vez; o cache gravado em `main` serve os PRs seguintes. Os três jobs correm em paralelo, então na corrida a frio só o primeiro a terminar grava — os outros registram `Unable to reserve cache with key`, que é o comportamento esperado e não falha o job.
