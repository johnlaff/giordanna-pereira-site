...
- **E-mail exibido**: `contato@giordannapereira.arq.br` desde o lançamento (#16), em constante única em `src/config.ts` (contatos, rodapé e fallback do formulário leem dela). O Email Routing entrega no Gmail dela, que não aparece no site: `tests/e2e/paginas.spec.ts` falha se `gmail.com` aparecer no HTML de qualquer rota.
...
- **Conteúdo da home e contatos**: hero, CTA e os quatro contatos vivem em `src/config.ts`, tipado, fora do CMS. Desde o #17 (pós-lançamento), Sobre (parágrafos, formação, credenciais) e Familiaridade (ferramentas por nível) estão numa file collection do Sveltia (`src/content/home/`, coleção "Página inicial"); em `src/config.ts` fica só a moldura deles (título e retrato), porque são os únicos textos da home que mudam com a carreira dela. Hero, CTA e contatos permanecem em código por decisão.
