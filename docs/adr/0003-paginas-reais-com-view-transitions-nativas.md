...
- A suíte de testes herdada do preview aponta para `#/projeto/<slug>` (singular); a migração da suíte, no ticket 1, troca as rotas para o esquema acima. Não há redirect de `/projeto/<slug>` a manter: o preview nunca foi publicado nem indexado. (remover no PR do ticket 1)
